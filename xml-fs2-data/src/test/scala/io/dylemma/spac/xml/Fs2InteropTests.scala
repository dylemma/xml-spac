...
class Fs2InteropTests extends AnyFunSpec with Matchers with XmlErrorHandlingBehaviors {
	describe("XML with fs2-data support") {
		describe("Exception 'spac' trace handling (but with no line numbers for inputs)") {
			...
			it should behave like xmlParserWithStringSource(Fs2DataSource.syncIO(_))
		}
	}
}
