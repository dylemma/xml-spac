...
object TransformerToPipe {
	def apply[F[_], A, B](t: Transformer[A, B], caller: SpacTraceElement): Pipe[F, A, B] = {
		stream => pullTransformed(stream, t.newHandler.asTopLevelHandler(caller)).stream
	}

	def pullTransformed[F[_], A, B](stream: Stream[F, A], transformer: Transformer[A, B]): Pull[F, B, Unit] = {
		Pull.suspend { Pull.pure[F, Transformer.Handler[A, B]](transformer.newHandler) } flatMap { handler =>
			pullTransformed(stream, handler)
		}
	}

	def pullTransformed[F[_], A, B](stream: Stream[F, A], handler: Transformer.Handler[A, B]): Pull[F, B, Unit] = {
