...
trait ParserBase[-In, +Out] { self =>
	...
	def unmapContext[In2](f: In2 => In): ParserBase[In2, Out] = new ParserBase[In2, Out]{
		def toIteratee(context: In2)(implicit ec: ExecutionContext) = self.toIteratee(f(context))
	}
}
...
trait ParserForContext[-C, +T] extends ParserBase[C, T] { self =>
	...
	/** Transform this parser by passing successful results through the transformation function `f` */
	@inline def map[U](f: T => U) = mapR(_ map f)
