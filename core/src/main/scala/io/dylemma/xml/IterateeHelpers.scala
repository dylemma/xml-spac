...
trait IterateeHelpers {
	...
	/** Create a new Enumeratee that advances a "state" based on the values of the
		...
		*
		* @param f A `FolderFunction` that accepts values of `A` as input, updating an
		*          immutable `State` over time, possibly emitting results of type `B`.
		* @tparam State The type of the `State` object managed by `f`
		...
		*/
	def scanResultsWith[State, A, B](f: StreamScan[State, A, B])(implicit ec: ExecutionContext) = {
		type ORA = Option[Result[A]]
		// Wrap all of the inputs as a `Some`, then pass a `None` just before the EOF
		val wrapEOF = Enumeratee.mapInputFlatten[Result[A]].apply[ORA] {
			case Input.EOF => Enumerator.enumInput(Input.El[ORA](None)) >>> Enumerator.enumInput(Input.EOF)
			case input => Enumerator.enumInput(input.map(Some(_)))
		}
		wrapEOF ><> Enumeratee.scanLeft[ORA].apply[(State, Result[B])](f.init -> Empty) { (stateAndResult, next) =>
			...
			next match {
				...
				case Some(e: Error) => f.init -> e
				// fold the input into the state
				case Some(Success(input)) => f.fold(state, input)
				case None => f.init -> f.finish(state)
			}
		} ><> Enumeratee.map(_._2)
	}
}
